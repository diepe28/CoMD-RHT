...
static void computeVcm_Producer(SimFlat* s, real_t vcm[3]);
static void computeVcm_Consumer(SimFlat* s, real_t vcm[3]);
...
void createFccLattice_Producer(int nx, int ny, int nz, real_t lat, SimFlat* s) {
   const real_t *localMin = s->domain->localMin; // alias
   const real_t *localMax = s->domain->localMax; // alias
...
void createFccLattice_Consumer(int nx, int ny, int nz, real_t lat, SimFlat* s) {
   const real_t *localMin = s->domain->localMin; // alias
   const real_t *localMax = s->domain->localMax; // alias
...
void setTemperature_Producer(SimFlat* s, real_t temperature) {
    // set initial velocities for the distribution
    for (int iBox = 0; iBox < s->boxes->nLocalBoxes; ++iBox) {
        for (int iOff = MAXATOMS * iBox, ii = 0; ii < s->boxes->nAtoms[iBox]; ++ii, ++iOff) {
            int iType = s->atoms->iSpecies[iOff];
...
void setTemperature_Consumer(SimFlat* s, real_t temperature) {
    // set initial velocities for the distribution
    for (int iBox = 0; iBox < s->boxes->nLocalBoxes; ++iBox) {
        for (int iOff = MAXATOMS * iBox, ii = 0; ii < s->boxes->nAtoms[iBox]; ++ii, ++iOff) {
            int iType = s->atoms->iSpecies[iOff];
...
void computeVcm_Producer(SimFlat* s, real_t vcm[3]) {
    real_t vcmLocal[4] = {0., 0., 0., 0.};
    real_t vcmSum[4] = {0., 0., 0., 0.};
...
void computeVcm_Consumer(SimFlat* s, real_t vcm[3]) {
    real_t vcmLocal[4] = {0., 0., 0., 0.};
    real_t vcmSum[4] = {0., 0., 0., 0.};
